...
export const addLikeForPost =
  (userID: number, postID: number) => async (dispatch: AppDispatch) => {
    try {
      return axios
        .post(`api/posts/like/`, { user: userID, post: postID })
        .then((res: any) => {
          console.log("res", res);
...
export const addBookmarkForPost =
  (postID: number) => async (dispatch: AppDispatch) => {
    try {
      return axios
        .post(`api/user/create-user-bookmarks/`, { post: postID })
        .then((res: any) => {
          return res.data.data;
        });
    } catch (error) {
...
export const deleteBookmarkForPost =
  (postID: number) => async (dispatch: AppDispatch) => {
    try {
      return axios
        .delete(`api/user/delete-user-bookmarks/${postID}/`, { post: postID })
        .then((res: any) => {
          return res.data;
        });
    } catch (error) {
...
export const addCommentForPost =
  (userID: number, postID: number, comment: string) =>
