      ...
      <div className="post-vertical">
        ...
        <DialogContent className="dialogContent" sx={{background: toggle ? "black" :"white" , padding:"50px 24px" }}>
          {postData.map((post: any) => {
            const isAlreadyLiked = post.like_post.some((ele:any)=>ele?.user?.id === props?.userDetails?.id);
            const likeCount = post?.like_post?.length || 0;
            ...
            return (
              ...
              >
                ...
                {!props.loading && post.type === 'image' && (
                  <Box sx={{ textAlign: 'center' }}>
                    <img
                      src={post.media}
                      alt="Not Displayed"
                      onClick={() => startFullScreenView(post)}
                      style={{ borderRadius: 18, width: '95%' }}
                    />
                  </Box>
                )}

                {!props.loading && post.type === 'video' && (
                  <Box sx={{ textAlign: 'center' }}>
                    <video
                      onClick={() => startFullScreenView(post)}
                      width="90%"
